...
- `RegressionMSR` now computes its `"msr"` adjustment as a closed-form Horvitz-Thompson correction on the coalitions it already sampled, instead of re-sampling an independent `CoalitionSampler` and running a second Monte Carlo pass (`SHAPIQ.approximate`) on the residual game; the two are numerically identical (matches to `1e-10`), and this removes the extra sampling/estimation round that made MSR's adjustment step the slow part of `approximate()`. This fast path only covers `index in {"SV", "BV"}`; any other index delegates the whole call to `ProxySHAP.approximate()` before sampling or fitting anything. `approximate()` now also sets `train_residual_ratio` and `correction_norm` diagnostics that quantify how much of the correction actually survives the fit, matching the reference implementation's own raw (baseline-inclusive) definitions exactly (see the class docstring; :cite:t:`Witter.2025`). This also corrects the v1.5.0 entry above, which calls `"msr"`/`"svarm"`/`"kernel"` uniformly "unbiased": the `k=1` all-samples variant `RegressionMSR` implements is consistent, not unbiased — the paper's unbiased variant needs `k >= 2` cross-fitting, which is not implemented here.
- **Behavior change:** `RegressionMSR`'s default `sampling_weights` (when `index in {"SV", "BV"}`) changed from the generic bowl-shaped `1 / (s * (n - s))` `Approximator` default to :cite:t:`Witter.2025`'s own sampling kernel, converted from the reference's per-coalition density to shapiq's per-size probability-mass convention (`regressionMSR/estimators/regMSR.py:108-150`; see the class docstring and `run_logs/bench_msr/gap_diag/GAP_REPORT.md`). An explicit `sampling_weights=` argument still overrides this default, and the fallback for any other index is unchanged. This is a real accuracy improvement, not just a docs change: on the 8-dataset benchmark grid, the new default's pooled ratio-to-reference error is 0.972 (CI [0.841, 1.12]) vs. the old default's 1.32 (CI [1.15, 1.51]) — about 25% lower error at large budgets, because the old bowl-shaped default was simply the wrong sampling scheme for this estimator (`run_logs/bench_msr/REPORT_v2.md`).
