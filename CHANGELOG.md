...
- adds `QuadratureTreeSHAP` in `shapiq.tree.quadrature`: a quadrature-based path-dependent tree explainer (Wettenstein et al., 2026; Li et al., 2026) computing Shapley values and **any-order** Shapley interactions (`"SV"`, `"SII"`, `"k-SII"`, plus the Banzhaf indices `"BV"`/`"BII"` via point evaluation) that is numerically exact in float64 at any tree depth and any number of features per decision path, resolving the instability of the polynomial explainers [#545](https://github.com/mmschlk/shapiq/issues/545). Ships a C++ kernel that benchmarks faster than the existing explainers at every measured setting: ~2× vs `LinearTreeSHAP` for Shapley values and 100×+ vs `TreeSHAPIQ` for interactions, with order-4 interactions in milliseconds. Ensembles are computed in a single batched C++ kernel invocation over a shared feature space, which benchmarks at parity with XGBoost's native `pred_contribs` TreeSHAP on a single thread. The interaction output is **sparse**: the returned `InteractionValues` enumerates exactly the interactions whose features co-occur on at least one decision path (all others are structurally zero), so wide models never materialize the dense `C(n_features, order)` enumeration — e.g. order-3 interactions of a 1000-feature forest are 89 co-occurring subsets instead of a 1.7e8-wide dense output.
For Shapley computation we keep a dense per-feature block, mirroring the dense-fast/sparse-general split of the interventional kernels. `QuadratureTreeSHAP` is the **default path-dependent algorithm** of `TreeExplainer`, computing all supported indices (`"SV"`, `"SII"`, `"k-SII"`, `"BV"`, `"BII"`) at any order natively.As such we have removed the woodelf dependency for path-dependent Banzhaf indices.
- harmonizes the tree computation algorithms: `LinearTreeSHAP`, `TreeSHAPIQ`, `QuadratureTreeSHAP`, and `InterventionalTreeSHAPIQ` now all validate their own models, accept ensembles as well as single trees (aggregating internally), take a `class_index` argument, and expose a common `explain(x)` entry point — each algorithm can be used independently of `TreeExplainer`, which now simply instantiates one explainer per mode. Their modules are named `computer` (`shapiq.tree.linear.computer`, `shapiq.tree.quadrature.computer`, `shapiq.tree.interventional.computer`), keeping `shapiq.tree.explainer` unambiguous as the user-facing `TreeExplainer` entry point.
- the interventional TreeSHAP-IQ C kernels now compute in float64 (previously float32 inputs, thresholds, and accumulation on the dense path); the three unused kernel entry points (`compute_interactions`, `compute_interactions_batched`, `compute_interactions_sparse`) were removed
...
- raises `TreeExplainer`'s interventional Woodelf routing cut-off from `n * m >= 100` to `n * m >= 100_000` (`n` explained instances × `m` reference rows). The old value predates the `InterventionalTreeSHAPIQ` improvements in [#590](https://github.com/mmschlk/shapiq/pull/590); re-benchmarking both backends end-to-end (sklearn random forests and XGBoost ensembles, 50–300 trees, depth 3–8, 10–50 features, `"SV"` and order-2 `"k-SII"`) puts the break-even between `n * m ≈ 1e4` (small ensembles) and `≈ 1e6` (large forests), with an asymmetric penalty: below the break-even Woodelf's fixed per-tree preprocessing makes it up to hundreds of times slower, while above it shapiq trails by a low single-digit factor near the boundary — `1e5` balances the worst-case losses; Woodelf still wins clearly on very large inputs (~5× at `n * m = 5e6`)
- vectorizes the k-SII aggregation (`aggregate_base_attributions`): all subset projections and Bernoulli weightings are built as array operations and summed per subset via integer-coded `bincount`, ~12–27× faster than the previous per-subset dict loop.
- adds numerical precision guards to the polynomial tree explainers: `LinearTreeSHAP` and `TreeSHAPIQ` now emit a `TreeNumericalPrecisionWarning` from 24 distinct features per decision path (measurably degrading values) and raise a `TreeNumericalPrecisionError` from 30 (unreliable values), instead of silently returning wrong attributions or crashing with an unexplained `LinAlgError` [#545](https://github.com/mmschlk/shapiq/issues/545). `TreeSHAPIQ`'s interpolation grid and `LinearTreeSHAP`'s N-matrix construction are now sized by the maximum number of distinct features per path (instead of depth-derived bounds), which removes latent `LinAlgError` crashes for deep trees whose individual paths stay feature-narrow and slightly improves their accuracy. `LinearTreeSHAP` additionally refuses trees containing unreachable (zero-cover) subtrees, whose infinite edge weights its polynomial representation cannot express (previously it silently returned NaN attributions for some instances, e.g. on CatBoost models with unreachable oblivious-tree regions)
- adds a Woodelf fast path to `shapiq.tree.TreeExplainer` (Nadel and Wettenstein, 2026; Wettenstein et al., 2026). Larger interventional inputs are computed by the vectorized Woodelf algorithms, and `TreeExplainer` now supports the Banzhaf indices `"BV"` and `"BII"`. `TreeExplainer.explain_X` explains all instances in one vectorized run and returns the new `InteractionValuesBatch`: a lazy sequence of one `InteractionValues` per instance whose `.values` attribute exposes the raw vectorized arrays (`Explainer.explain_X` is now typed as returning a `Sequence[InteractionValues]`). The fast path requires the new optional `tree` dependency group (`pip install shapiq[tree]` — `woodelf-explainer`); without it, the explainer falls back to the shapiq implementation with a `WoodelfNotAvailableWarning`. The new `backend` parameter (`"auto"`, `"woodelf"`, `"shapiq"`) overrides the automatic routing [#572](https://github.com/mmschlk/shapiq/pull/572)
...
- fixes `LeverageSHAP`'s sampling and budget accounting (all relative to the unreleased [#524](https://github.com/mmschlk/shapiq/pull/524) implementation): `pairing_trick` is now functional (`True`, the default, forces each sampled coalition's complement into the sample; `False` reproduces the paper's "without paired sampling" ablation) rather than a documented no-op, and `estimation_budget` is now exact in the default mode (`2 + 2 * ((min(budget, 2**n) - 2) // 2)`, no over- or undershoot), with `_find_c` no longer occasionally undershooting the full-enumeration budget at `budget == 2**n`. `LeverageSHAP` now defaults to `deterministic_counts=True`, fixing each coalition size's sample count to the expected value of the paper's Binomial draw (largest-remainder rounded) instead of drawing it at random, following the fixed-per-size design used by the paper's released implementation and reported experiments. The deterministic weights use the realized per-size inclusion probabilities; odd budgets round down to respect shapiq's budget ceiling (the released implementation rounds them up). Pass `deterministic_counts=False` for the random Binomial behavior (Algorithm 2) [#583](https://github.com/mmschlk/shapiq/pull/583)
...
- adds a `k_folds` parameter to `ProxySHAP` and `RegressionMSR` for out-of-fold proxy residuals, i.e. the cross-fitting variant of Witter et al. (2025). The default `k_folds=1` fits a single proxy on all sampled coalitions, which the paper recommends in practice and which keeps the estimator exact at full budget.
...
- fixes wrong explanations for XGBoost and CatBoost models on instances whose float64 feature values round across a split threshold in float32: both libraries cast prediction inputs to float32 before routing, which the converted trees now reproduce via `TreeModel.input_precision`/`cast_input` (LightGBM compares in float64 and is untouched; sklearn's thresholds are exact midpoints of adjacent float32 values, making its internal cast unobservable). Previously all tree explainers could explain a different leaf than the model actually predicts — on measured XGBoost `hist` models roughly half the instances routed differently somewhere in the ensemble
- fixes silently permuted attributions for trees whose feature-id set iterates in non-ascending order (e.g. features `{6, 14}`): `reduce_feature_complexity` assigned reduced ids in raw `set` iteration order while the returned `interaction_lookup` sorts players, pairing values with the wrong features in `TreeSHAPIQ` (and the new quadrature explainer before the fix; `LinearTreeSHAP` was unaffected). Efficiency still held, so nothing downstream could catch it; reduced ids are now assigned in sorted order
- `validate_tree_model` now returns trees the caller owns: `TreeModel` and list-of-`TreeModel` inputs are deep-copied, so constructing an explainer no longer mutates the passed-in model. Previously, `LinearTreeSHAP` (and therefore `TreeExplainer` with `index="SV"`, `max_order=1`) rewrote the caller's `features`/`feature_ids` in place via `reduce_feature_complexity`, corrupting the tree's feature metadata and producing silently wrong attributions when the same `TreeModel` was reused with the interventional explainer. `TreeSHAPIQ`'s now-redundant internal deepcopy was removed, and a list input containing non-`TreeModel` elements now raises a `TypeError` instead of silently returning an empty list [#544](https://github.com/mmschlk/shapiq/issues/544)
