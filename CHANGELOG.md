...
- adds `QuadratureTreeSHAP` in `shapiq.tree.quadrature`: a quadrature-based path-dependent tree explainer (Wettenstein et al., 2026; TreeGrad, 2026) computing Shapley values and **any-order** Shapley interactions (`"SV"`, `"SII"`, `"k-SII"`, plus the Banzhaf indices `"BV"`/`"BII"` via point evaluation) that is numerically exact in float64 at any tree depth and any number of features per decision path — resolving the instability of the polynomial explainers [#545](https://github.com/mmschlk/shapiq/issues/545). Ships a C++ kernel (required — a missing extension raises an `ImportError` at construction; the numpy reference implementation lives in the test suite as the differential oracle) that benchmarks faster than the existing explainers at every measured setting: ~2× vs `LinearTreeSHAP` for Shapley values and 100×+ vs `TreeSHAPIQ` for interactions, with order-4 interactions in milliseconds. Ensembles are computed in a single batched C++ kernel invocation over a shared feature space (no per-tree Python objects on the hot path), which benchmarks at parity with XGBoost's native `pred_contribs` TreeSHAP on a single thread. The interaction output is **sparse**: the returned `InteractionValues` enumerates exactly the interactions whose features co-occur on at least one decision path (all others are structurally zero), so wide models never materialize the dense `C(n_features, order)` enumeration — e.g. order-3 interactions of a 1000-feature forest are 89 co-occurring subsets instead of a 1.7e8-wide dense output; order 1 remains a dense per-feature block with unchanged (Shapley hot-path) performance, mirroring the dense-fast/sparse-general split of the interventional kernels. `QuadratureTreeSHAP` is the **default path-dependent algorithm** of `TreeExplainer`, computing all supported indices (`"SV"`, `"SII"`, `"k-SII"`, `"BV"`, `"BII"`) at any order natively — path-dependent Banzhaf indices no longer require the optional woodelf dependency, and the order- and size-based routing rules between the previous path-dependent implementations were removed. Also directly available via `shapiq.tree.QuadratureTreeSHAP`
- harmonizes the tree computation algorithms: `LinearTreeSHAP`, `TreeSHAPIQ`, `QuadratureTreeSHAP`, and `InterventionalTreeSHAPIQ` now all validate their own models, accept ensembles as well as single trees (aggregating internally), take a `class_index` argument, and expose a common `explain(x)` entry point — each algorithm can be used independently of `TreeExplainer`, which now simply instantiates one explainer per mode. Their modules are named `computer` (`shapiq.tree.linear.computer`, `shapiq.tree.quadrature.computer`, `shapiq.tree.interventional.computer`), keeping `shapiq.tree.explainer` unambiguous as the user-facing `TreeExplainer` entry point; the public class imports (`from shapiq.tree import ...`) are unchanged
