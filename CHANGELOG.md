...
- `RegressionMSR` now computes its `"msr"` adjustment as a closed-form Horvitz-Thompson correction on the coalitions it already sampled, instead of re-sampling an independent `CoalitionSampler` and running a second Monte Carlo pass (`SHAPIQ.approximate`) on the residual game; the two are numerically identical (matches to `1e-10`), and this removes the extra sampling/estimation round that made MSR's adjustment step the slow part of `approximate()`. `approximate()` now also sets `train_residual_ratio` and `correction_norm` diagnostics that quantify how much of the correction actually survives the fit (see the class docstring; :cite:t:`Witter.2025`). This also corrects the v1.5.0 entry above, which calls `"msr"`/`"svarm"`/`"kernel"` uniformly "unbiased": the `k=1` all-samples variant `RegressionMSR` implements is consistent, not unbiased — the paper's unbiased variant needs `k >= 2` cross-fitting, which is not implemented here.
