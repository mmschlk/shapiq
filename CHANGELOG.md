...
- fixes `LeverageSHAP`'s sampling and budget accounting: `pairing_trick` is now functional (`True`, the default, forces each sampled coalition's complement into the sample; `False` reproduces the paper's "without paired sampling" ablation) rather than being silently ignored, and `estimation_budget` is now exact in the default mode (`2 + 2 * ((min(budget, 2**n) - 2) // 2)`, no over- or undershoot), with `_find_c` no longer occasionally undershooting the full-enumeration budget at `budget == 2**n`. `LeverageSHAP` also now defaults to `deterministic_counts=True`, fixing each coalition size's sample count to the expected value of the paper's Binomial draw (largest-remainder rounded) instead of drawing it at random, matching the paper's released implementation and reported experiments — **this changes which coalitions are sampled (and therefore the estimate) compared to v1.6.0** for any caller that doesn't pass `deterministic_counts`; pass `deterministic_counts=False` to keep the old random Binomial behavior (Algorithm 2) [#505](https://github.com/mmschlk/shapiq/issues/505)
