...
- adds the `LeverageSHAP` approximator in `shapiq.approximator.regression` for Shapley value estimation via leverage score sampling (Musco and Witter, 2025) [#524](https://github.com/mmschlk/shapiq/pull/524). `LeverageSHAP` now defaults to `deterministic_counts=True`, fixing each coalition size's sample count to the expected value of the paper's Binomial draw (largest-remainder rounded) instead of drawing it at random, matching the paper's released implementation and reported experiments; `deterministic_counts=False` keeps the original random Binomial variant (Algorithm 2). `pairing_trick` is now functional (`True`, the default, forces each sampled coalition's complement into the sample; `False` reproduces the paper's "without paired sampling" ablation) rather than being silently ignored. `estimation_budget` is now exact in the default mode (`2 + 2 * ((min(budget, 2**n) - 2) // 2)`, no over- or undershoot), and `_find_c` no longer occasionally undershoots the full-enumeration budget at `budget == 2**n` [#505](https://github.com/mmschlk/shapiq/issues/505)
