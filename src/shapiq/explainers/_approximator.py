...
class Approximator(Explainer[Array, Game[Array]], ABC):
    ...
    spent: int
    units_done: int
    _render: Callable[[Array], Array]
    _unit_length: int
    _prelude_masks: Array | None
    ...
    def sample(self, budget: int) -> Approximator:
        ...
        bank = remaining - rows
        self._checkpoint(evidence, bank)
        return self._evolve(
            state=evidence,
    ...
    def _sample_deduplicated(self, budget: int) -> Approximator:
        ...
        quiet_units = 0
        while remaining > 0 and quiet_units < _STALL_UNITS:
            ...
            for unit in range(n_request):
                ...
                kept_units = unit + 1
                if charge >= remaining or quiet_units >= _STALL_UNITS:
                    break
        ...
        if remaining > 0:
            msg = (
                ...
                f"coalitions were found (a game with {self.game.n_players} players has "
                f"at most 2**{self.game.n_players} distinct coalitions); evidence "
                "gathered so far remains valid"
            )
            warnings.warn(msg, SamplingStallWarning, stacklevel=3)
        if evidence is not self.state:
            self._checkpoint(evidence, remaining)
        return self._evolve(state=evidence, units_done=units_done, spent=spent, bank=remaining)

    def _checkpoint(self, evidence: SamplingState[Array], bank: int) -> None:
    ...
    def rollback(self, steps: int = 1) -> Approximator:
    ...
    def _at_state(self, state: ApproximationState) -> Approximator:
        ...
        if not isinstance(state, SamplingState):
            return self._evolve(state=state, units_done=0, spent=0, bank=0)
        ...
        return self._evolve(state=state, units_done=units_done, spent=spent, bank=bank)
