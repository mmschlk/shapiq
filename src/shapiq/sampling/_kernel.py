...
class CoalitionSampler(Sampler):
    """Base sampler whose draws are coalitions, full stop.

    A draw is one coalition mask; consuming approximators enter drawn
    coalitions directly as sample rows. The antithesis of a coalition is
    its complement — wrap the sampler in ``PairedSampler`` to draw both,
    which is variance-reducing exactly when the distribution is
    complement-symmetric. Draw ``unit_index`` derives from
    ``fold_in(random_state, unit_index)``, so draws are order-free.
    Concrete samplers fix the distribution: ``SizeKernelSampler`` for
    size-based distributions and ``ProductKernelSampler`` for product
    measures; sampling proportional to a regression kernel is what lets an
    unweighted least squares fit carry the correct implicit weighting.
    """

    def draws(self, unit_indices: Array) -> Array:
        """Return one drawn coalition mask per unit index."""
        keys = jax.vmap(lambda index: jax.random.fold_in(self._key, index))(
            jnp.asarray(unit_indices),
        )
        return jax.vmap(self._draw_from_key)(keys)

    def antithetic(self, draws: Array) -> Array:
        """Return the complement coalitions."""
        return ~draws

    @abstractmethod
    def _draw_from_key(self, unit_key: Array) -> Array:
        """Return the coalition mask one unit key stands for."""


class SizeKernelSampler(CoalitionSampler):
    ...
    size ``0..n_players``. A draw first picks a size from the normalized
    weights and then a uniform coalition of that size, so a coalition of
    size ``t`` appears with probability proportional to
    ``size_weights[t] / comb(n_players, t)``; sizes with zero weight are
    never drawn. Mind the distinction from a per-coalition kernel weight
    ...
    def _draw_from_key(self, unit_key: Array) -> Array:
        ...
        return positions < sizes[..., None]


class ShapleyKernelSampler(SizeKernelSampler):
    """Sampler drawing coalitions from the Shapley kernel size distribution.

    Each draw picks a coalition whose size ``t`` follows the normalized
    Shapley kernel distribution ``p(t) proportional to 1 / (t * (n - t))``
    over sizes ``1`` to ``n - 1`` and whose members are uniform given the
    size, so every drawn coalition appears with probability proportional to
    its kernel weight: the size weights are the per-coalition Shapley
    kernel times ``comb(n, t)``. Wrap the sampler in ``PairedSampler`` to
    draw the complement alongside, which follows the same distribution by
    kernel symmetry.
    """
...
class ProductKernelSampler(CoalitionSampler):
    """Sampler drawing coalitions from a product measure by membership flips.

    Every player joins a drawn coalition independently with probability
    ...
    membership flips are exact for any number of players, and the
    mechanism extends to player-specific probabilities (a planned
    capability), which no size-based distribution can express. The product
    measure is complement-symmetric only at ``p = 0.5``, so pairing is
    reserved for the uniform case.
    """
        ...
        *,
        ...
        Args:
            ...
            p: Probability with which each player independently joins a
                drawn coalition. Must satisfy ``0 < p < 1``.
            target_shape: Shape of the explanation targets, matching the
                game's target shape.
            share_samples: Policy for sharing draws across explanation-target
                axes. ``False`` draws independently per target; ``True``
        ...
        if not 0.0 < p < 1.0:
            msg = (
                f"p must satisfy 0 < p < 1, got {p}; at the limits every drawn "
...
class BanzhafKernelSampler(ProductKernelSampler):
    ...
    realized by independent fair membership coin flips per player, so drawn
    rows enter an unweighted least squares fit of a uniform-kernel index
    with the correct implicit weighting. Wrap the sampler in
    ``PairedSampler`` to draw the complement alongside, which is uniform by
    symmetry.
    """
