...
def load_iris() -> tuple[pd.DataFrame, pd.Series]:
    ...
    Returns:
        The Iris dataset as pandas objects ``(X, y)``.

    Notes:
        The dataset contains 150 samples with 4 features and 3 classes.
        All continuous, no missing values. Target label-encoded to 0-based integers.

    Example:
        >>> from shapiq_games.datasets import load_iris
        >>> x_data, y_data = load_iris()
        >>> print(x_data.shape, y_data.shape)
        ((150, 4), (150,))

    """
    X, y = iris(return_X_y=True, as_frame=True)
    y = pd.Series(LabelEncoder().fit_transform(y.astype(str)), name="target")
    return X, y


def load_monks1() -> tuple[pd.DataFrame, pd.Series]:
    """Load MONK's Problem 1 dataset from the UCI Machine Learning Repository.

    Original source: https://archive.ics.uci.edu/dataset/70/monk+s+problems

    Note:
        Requires the ``ucimlrepo`` package (``pip install ucimlrepo``).

    Returns:
        MONK's Problem 1 dataset as pandas objects ``(X, y)``.

    Notes:
        The dataset contains 556 samples with 6 features and 2 classes.
        All integer-encoded categorical features; target label-encoded to 0-based integers.

    Example:
        >>> from shapiq_games.datasets import load_monks1
...
def load_monks2() -> tuple[pd.DataFrame, pd.Series]:
    """Load MONK's Problem 2 dataset from the UCI Machine Learning Repository.
    ...
    Returns:
        MONK's Problem 2 dataset as pandas objects ``(X, y)``.

    Notes:
        The dataset contains 601 samples with 6 features and 2 classes.
        All integer-encoded categorical features; target label-encoded to 0-based integers.

    Example:
        >>> from shapiq_games.datasets import load_monks2
...
def load_monks3() -> tuple[pd.DataFrame, pd.Series]:
    """Load MONK's Problem 3 dataset from the UCI Machine Learning Repository.
    ...
    Returns:
        MONK's Problem 3 dataset as pandas objects ``(X, y)``.

    Notes:
        The dataset contains 554 samples with 6 features and 2 classes.
        All integer-encoded categorical features with ~5% label noise;
        target label-encoded to 0-based integers.

    Example:
        >>> from shapiq_games.datasets import load_monks3
