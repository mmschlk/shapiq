...
class GameConfigurator:
    """Factory class creating configurators for benchmarked games.
    It returns the configurations for a given game class.

    Attributes:
        game_names (list[str]): List of game class names to configure.
        configurations (list[dict[str, str]]): Dictionary of configurations for each game class.
        iteration_parameter (str): Parameter to iterate over for creating multiple games.
        n_players (int): Number of players in the game.
        precompute (bool): Flag indicating whether to precompute the game or not.
    ...
    def __init__(
        ...
        *,
        ...
        """Initializes the GameConfigurator with the given game class and configuration.
        ...
        if isinstance(game_class_or_name, str):
            ...
            self.game_class = game_class_or_name
            self.game_name = GAME_CLASS_TO_NAME_MAPPING[self.game_class]
        if config_path is None:
            self.config_path = f"{self.game_name}_config.json"
        else:
            self.config_path = config_path
        self.configurations = configurations
        self.iteration_parameter = iteration_parameter
        self.iteration_parameter_values_names = iteration_parameter_values_names
        self.iteration_parameter_values = iteration_parameter_values
        self.n_players = n_players
        self.precompute = precompute

    def to_json(self) -> None:
        """Saves the GameConfigurator to a JSON file."""
        import json

        data = {
            "game_name": self.game_name,
            "game_class": self.game_class.__qualname__,
            "configurations": self.configurations,
            "iteration_parameter": self.iteration_parameter,
            "iteration_parameter_values_names": self.iteration_parameter_values_names,
            "iteration_parameter_values": self.iteration_parameter_values,
    ...
    def from_json(self, file_path: str) -> None:
        ...
        self.game_class = data["game_class"]
    ...
    def add_configuration(self, config: dict[str, Any]) -> None:
        """Adds a configuration to the GameConfigurator.

        Args:
            config (dict[str, Any]): The configuration dictionary to add.
        """
        self.configurations.append(config)
        self.to_json()

    def delete_configuration(self, index: int) -> None:
        """Deletes a configuration from the GameConfigurator.
    ...
    def choose_configuration(self, index: int) -> dict[str, Any]:
        ...
        Args:
            index (int): The index of the configuration to choose.
        """
        if 0 <= index < len(self.configurations):
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"GameConfigurator(game_class={self.game_class.__qualname__}, "
...
BENCHMARK_CONFIGURATIONS: dict[Game, list[dict[str, Any]]] = {
    ...
    BikeSharingClusterExplanation: [
        {
            "configurations": [
                {"cluster_method": "kmeans", "score_method": "silhouette_score"},
                {
                    ...
                    "score_method": "calinski_harabasz_score",
                },
            ],
...
GAME_NAME_TO_CLASS_MAPPING = {
    ...
    "ForestFiresLocalXAI": ForestFiresLocalXAI,
    "IndependentLinear60LocalXAI": IndependentLinear60LocalXAI,
    "Corrgroups60LocalXAI": Corrgroups60LocalXAI,
    "NHANESILocalXAI": NHANESILocalXAI,
    "RealEstateLocalXAI": RealEstateLocalXAI,
    "CommunitiesAndCrimeLocalXAI": CommunitiesAndCrimeLocalXAI,
    "BreastCancerLocalXAI": BreastCancerLocalXAI,
    "ResNet18w14Superpixel": ImageClassifier,
    "ViT3by3Patches": ImageClassifier,
    ...
    "MicroresponseLocalXAI": MicroresponseLocalXAI,
    "AnnealingLocalXAI": AnnealingLocalXAI,
    "ArrhythmiaLocalXAI": ArrhythmiaLocalXAI,
    "ChessLocalXAI": ChessLocalXAI,
    "CondindLocalXAI": CondindLocalXAI,
    "CrossLocalXAI": CrossLocalXAI,
    "DisjunctLocalXAI": DisjunctLocalXAI,
    "GroupLocalXAI": GroupLocalXAI,
    "HepatitisLocalXAI": HepatitisLocalXAI,
    "IonosphereLocalXAI": IonosphereLocalXAI,
    "MushroomLocalXAI": MushroomLocalXAI,
    "NurseryLocalXAI": NurseryLocalXAI,
    "SoybeanLocalXAI": SoybeanLocalXAI,
    "SphereLocalXAI": SphereLocalXAI,
    "ThyroidLocalXAI": ThyroidLocalXAI,
    "WineQualityLocalXAI": WineQualityLocalXAI,
    "XorLocalXAI": XorLocalXAI,
    "ZooLocalXAI": ZooLocalXAI,
}
...
APPROXIMATION_BENCHMARK_PARAMS: dict[Approximator, tuple[str]] = {}
...
def get_game_class_from_name(game_name: str) -> Game:
...
def get_name_from_game_class(game_class: Game) -> str:
