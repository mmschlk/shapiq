...
def transform_interactions_to_shap(interaction_values, interaction_lookup):
    ...
    transformed_values[interaction_lookup[()]] = interaction_values[
        interaction_lookup[()]
    ]
...
if __name__ == "__main__":
    ...
    basis_stoch = basis_generator.generate_stochastic_explanation_basis(
        n_explanation_terms=12
    )
    ...
    kernelshap = solve_least_squares(
        regression_matrix_kernelshap, regression_weights, game_values
    )

    # GAX
    regression_matrix_gax2add = create_regression_matrix(coalitions, basis_order2)
    interaction_values = solve_least_squares(
        regression_matrix_gax2add, regression_weights, game_values
    )
    shapleygax2 = transform_interactions_to_shap(interaction_values, basis_order2)[
        : n_players + 1
    ]
    ...
    interaction_values = solve_least_squares(
        regression_matrix_gax3add, regression_weights, game_values
    )
    shapleygax3 = transform_interactions_to_shap(interaction_values, basis_order3)[
        : n_players + 1
    ]
    ...
    interaction_values = solve_least_squares(
        regression_matrix_gax4add, regression_weights, game_values
    )
    shapleygax4 = transform_interactions_to_shap(interaction_values, basis_order4)[
        : n_players + 1
    ]

    # GAX stochastic
    regression_matrix_gaxstoch = create_regression_matrix(coalitions, basis_stoch)
    interaction_values_stoch = solve_least_squares(
        regression_matrix_gaxstoch, regression_weights, game_values
    )
    shapleygax_stoch = transform_interactions_to_shap(
        interaction_values_stoch, basis_stoch
    )[: n_players + 1]
    ...
    M_hat = (
        np.linalg.inv(regression_matrix_kernelshap.T @ regression_matrix_kernelshap)
        @ regression_matrix_kernelshap.T
    )
    pseudo_inverse_gax = np.linalg.pinv(regression_matrix_gax2add)

    weighted_regression_matrix_2add = (
        np.sqrt(kernel_weights[:, None]) * regression_matrix_gax2add
    )
    weighted_regression_matrix_3add = (
        np.sqrt(kernel_weights[:, None]) * regression_matrix_gax3add
    )
