...
| Sampling rework (merged issue 9) | **landed 2026-07-22** (`a498e5f5`, ADR 0014): stateless draw sources, one loop on `Approximator` with banked whole-unit budgets and `(rows, bank)` checkpoints (history always on — F3 closed), dedup as an in-loop policy (batched charge scan: dedup regression 62.5 → 9.5 ms). **Remaining: slice 6** — bounded-batch `_call_game` policy shared with `ExactExplainer`'s 2^n sweep, `ChunkedMaskedPredictor` slims to torch policy; permutation-dedup charge scan is python-per-row (31 vs 27 ms — revisit if it grows) | issue-9 | slice 6 open; SHAP-IQ port validates next |
