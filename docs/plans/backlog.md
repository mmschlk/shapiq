...
| --- | --- | --- | --- |
| Value-function rebuild ("world A") | clean-slate re-centering: explanations ARE games, (game, measure) at the center, three tiers with bases, process on frozen policies — design + evidence + arc plan in issue-14; supersedes-in-direction most rows below if it proceeds | issue-14 | design converged 2026-07-24; rebuild not started |
...
| Sampling-state addressability (remainder) | landed 2026-07-21 (ADR 0013): `unique()`/`packed_keys()`/`key_index()` on the state, dedup as a consumer, `LawfulSampler` with pairing symmetrization. Remaining: wire Regression's solve to the unique view (√count weighting — proven equivalent, smaller solves), exactness-strata design when the border trick ports, law's first estimator consumer = sampled k-SII port | issue-13 | remainder open |
| Sampling rework (merged issue 9) | **landed 2026-07-22** (`a498e5f5`, ADR 0014): stateless draw sources, one loop on `Approximator` with banked whole-unit budgets and `(rows, bank)` checkpoints (history always on — F3 closed), dedup as an in-loop policy (batched charge scan: dedup regression 62.5 → 9.5 ms). Follow-up same day: admission policy extracted to `approximators/_deduplication.py` consuming `SamplingState.key_index()`; golden streams bit-identical across the cut. **Remaining: slice 6** — bounded-batch `_call_game` policy shared with `ExactExplainer`'s 2^n sweep, `ChunkedMaskedPredictor` slims to torch policy; permutation-dedup charge scan is python-per-row (~25-33 ms run noise — revisit if it grows) | issue-9 | slice 6 open; SHAP-IQ port validates next |
