...
def _convert_xgboost_tree_as_df(
    ...
    with pd.option_context('future.no_silent_downcasting', True):
        return TreeModel(
            children_left = tree_df['Yes'].replace(convert_node_str_to_int)\
                .infer_objects(copy=False).fillna(-1).astype(int).values,
            children_right = tree_df['No'].replace(convert_node_str_to_int)\
                .infer_objects(copy=False).fillna(-1).astype(int).values,
