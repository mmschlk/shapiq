...
class ExplanationBasisGenerator:
    ...
    def _init_kernel_weights(self) -> np.ndarray:
        """Initializes the KernelSHAP weights.
        ...
        for coalition_size in range(self.n + 1):
            if coalition_size <= 1 or coalition_size == self.n:
                weight_vector[coalition_size] = 100_000_000
            else:
    ...
    def generate_kernelshap_explanation_basis(self, n_explanation_terms, conjugate=False):
        sampling_weights = self._init_kernel_weights()
        sampler = CoalitionSampler(self.n, sampling_weights, pairing_trick=True)
        # get the explanation basis
        sampler.sample(n_explanation_terms)
        explanation_basis = {}
        for coalition_pos, coalition in enumerate(sampler.coalitions_matrix):
            explanation_basis[tuple(np.where(coalition)[0])] = coalition_pos
        return explanation_basis

    def generate_partial_explanation_basis(self, n_explanation_terms, sizes_to_exclude=None):
        perm = list(self.N)
    ...
    def generate_stochastic_explanation_basis(
        self, n_explanation_terms, sizes_to_exclude=None, conjugate=False
    ):
        explanation_basis = {}
        S_size = 0
        S_pos = 0
