...
def get_parent_array(
    ...
    """Combines the left and right children of a tree to a parent array. The parent of the
        root node is -1.

    Args:
        children_left (np.ndarray[int]): The left children of each node in a tree.
            Leaf nodes are -1.
        children_right (np.ndarray[int]): The right children of each node in a tree.
            Leaf nodes are -1.
...
def get_conditional_sample_weights(
    ...
    Args:
        sample_count (np.ndarray[int]): The counts of the instances going through each node.
