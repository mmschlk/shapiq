...
def aggregate_base_attributions(
...
) -> tuple[np.ndarray, dict[tuple[int, ...], int], dict[tuple[int, ...], float], str, int]:
    ...
    Returns:
        A tuple containing:
            - The aggregated interaction values as a numpy array.
            - A lookup dictionary mapping interactions to their index in the values vector.
    ...
    aggregated_values = np.zeros(len(transformed_interactions), dtype=float)
    for pos, (interaction, interaction_value) in enumerate(transformed_interactions.items()):
        interaction_lookup[interaction] = pos
