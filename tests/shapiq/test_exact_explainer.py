...
def test_unsupported_indices_are_rejected():
    with pytest.raises(TypeError, match=r"not the string 'SII'"):
...
def test_index_classes_expose_semantics_and_explainers_resolve_orders():
    assert SII.order_semantics == "coverage"
    assert STII.order_semantics == "identity"
    assert FSII.order_semantics == "identity"
    assert ExactExplainer(game_from(quadratic_from_masks), SV).order == 1
    assert ExactExplainer(game_from(quadratic_from_masks), SII, order=3).order == 3
    with pytest.raises(TypeError, match="pass order= to the explainer"):
        ExactExplainer(game_from(quadratic_from_masks), SII)
    with pytest.raises(ValueError, match="at least 1"):
        ExactExplainer(game_from(quadratic_from_masks), SII, order=0)
