...
from shapiq import CallableGame, ExactExplainer, PermutationSamplingSV
...
@pytest.mark.parametrize(("index", "weight_factory"), [("SII", sii_weight), ("BII", bii_weight)])
def test_exact_base_interactions_match_brute_force(index, weight_factory):
    explanation = ExactExplainer(game_from(cubic_from_masks), index, order=2).explain()
...
def test_unsupported_indices_are_rejected():
    with pytest.raises(ValueError, match="does not support"):
        ExactExplainer(game_from(quadratic_from_masks), "k-SII")
    with pytest.raises(ValueError, match="SV requires order == 1"):
        ExactExplainer(game_from(quadratic_from_masks), "SV", order=2)
