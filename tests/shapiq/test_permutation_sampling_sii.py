...
def test_pair_estimates_are_exact_for_quadratic_games():
    explanation = (
        PermutationSampling(game_from(quadratic_from_masks), SII, order=2, random_state=0)
...
def test_pair_estimates_converge_to_brute_force_sii():
    explanation = (
        PermutationSampling(game_from(cubic_from_masks), SII, order=2, random_state=1)
...
def test_order_one_estimates_match_sv_approximator_exactly():
    n_walks = 30
    sii = PermutationSampling(game_from(cubic_from_masks), SII, order=2, random_state=4)
...
def test_explaining_before_pair_coverage_raises():
    approximator = PermutationSampling(
        game_from(quadratic_from_masks), SII, order=2, random_state=0
    )
    with pytest.raises(InsufficientSamplesError):
...
def test_pending_samples_are_masked_until_their_walk_completes():
    def make():
        return PermutationSampling(
            game_from(quadratic_from_masks), SII, order=2, random_state=5
        )
...
def test_sampling_is_invariant_to_budget_splits():
    def make():
        return PermutationSampling(
            game_from(quadratic_from_masks), SII, order=2, random_state=11
        )
...
def test_shared_samples_across_scaled_targets():
    ...
    approximator = PermutationSampling(game, SII, order=2, random_state=2, share_samples=True)
