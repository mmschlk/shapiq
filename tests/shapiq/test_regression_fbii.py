...
def test_recovers_quadratic_games_exactly_once_identified():
    ...
    assert jnp.allclose(estimate[()], 0.0, atol=1e-5)


def test_order_one_converges_to_the_banzhaf_value():
    exact = order_one(ExactExplainer(game_from(cubic_from_masks), BV()).estimate())
...
def test_converges_to_the_exact_faithful_banzhaf_interactions():
    exact = ExactExplainer(game_from(cubic_from_masks), FBII(order=2)).estimate()
