...
def test_resnet_model_class(image_and_path):
    """Tests the creation of the ResNet Model class."""
    test_image, _ = image_and_path
...
def test_image_classifier_game_resnet(image_and_path):
...
def test_vit_model_class(image_and_path):
    ...
    assert vit_model(coalitions=np.ones(16))[0] >= 0.9195  # check that call works
    assert vit_model(coalitions=np.zeros(16))[0] == vit_model.empty_value
...
def test_image_classifier_game_vit(image_and_path):
