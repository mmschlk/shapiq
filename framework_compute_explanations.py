...
def _compare_explanations(_explanation_mi: dict, _explanation: dict) -> None:
    for _feature_set, _exp_val in _explanation.items():
        _exp_val_mi = _explanation_mi[_feature_set]
        similar = abs(_exp_val - _exp_val_mi) < 1e-3
        assert similar, f"Explanations are not similar: {(_exp_val, _exp_val_mi)}"
...
if __name__ == "__main__":
    ...
    results_mi_df.to_csv(os.path.join(RESULTS_DIR, f"{save_name}_explanations_mi.csv"), index=False)
