...
if __name__ == "__main__":
    RANDOM_STATE = 40
    GAMES_IDENTIFIER = ["AdultCensusLocalXAI"] #["SentimentAnalysisLocalXAI","ImageClassifierLocalXAI"]
    ...
    for game_identifier in GAMES_IDENTIFIER:
        ...
        for n_player_id in N_PLAYER_IDS:
            ...
            budget_steps = np.linspace(min(500,0.1*2**n_players), min(2**n_players, 20000), 10).astype(int)
            ...
            shapley_gax_kadd_lev = ShapleyGAX(n=n_players,explanation_basis=kadd,random_state=RANDOM_STATE,sampling_weights=leverage_weights_1)
            shapley_gax_kadd_lev.name = "ShapleyGAX-2ADD-Leverage1"
            shapley_gax_kadd_lev2 = ShapleyGAX(n=n_players,explanation_basis=kadd,random_state=RANDOM_STATE,sampling_weights=leverage_weights_2)
            ...
            index = "SV"
            order = 1
            save_path = "data/"+game_identifier+"_"+str(n_player_id)+"_results.json"
