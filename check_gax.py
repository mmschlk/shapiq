...
if __name__ == "__main__":
    ...
    stoch = explanation_basis.generate_stochastic_explanation_basis(n_explanation_terms=20)
    kadd1 = explanation_basis.generate_kadd_explanation_basis(max_order=1)

    PAIRING = True
    REPLACEMENT = False

    approximator_1 = ShapleyGAX(n=n_players, explanation_basis = kadd1, random_state=10, pairing_trick=PAIRING, replacement=REPLACEMENT)
    approximator_2 = ShapleyGAX(n=n_players, explanation_basis = kadd2, random_state=10, pairing_trick=PAIRING, replacement=REPLACEMENT)
    approximator_stoch = ShapleyGAX(n=n_players, explanation_basis = stoch, random_state=10, pairing_trick=PAIRING, replacement=REPLACEMENT)
    ...
    import numpy as np
    print(np.max(approximator_1._sampler.coalitions_matrix == approximator_2._sampler.coalitions_matrix))
    print(shap_1.values)
    print(shap_2.values)
